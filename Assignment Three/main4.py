...
def main():
	...
	epochs = [i for i in xrange(1,5)]
	data_vals = 2
	learn_rate = 0.01
	(net, results) = cte(train_loader, validation_loader, F.cross_entropy, learn_rate, 0.9, epochs = 4)
	accuracy = test(net,test_loader)
	print "Accuracy on testing set: ",accuracy
	plt.plot(epochs, [item[0] for item in results], label = 'Loss Part 4')
	plt.xlabel('Epoch')
	plt.ylabel('Avg. Loss')
